...
function SectorRow({ s }) {
  ...
  return (
    <div style={{ padding: "6px 0", borderBottom: `1px solid ${T.line2}` }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <span style={{ fontSize: 12, fontWeight: 700, width: 100 }}>{s.k}</span>
        <div style={{ flex: 1 }}><BiasMeter score={s.bias} /></div>
        <span style={{ fontFamily: T.mono, fontSize: 11.5, fontWeight: 700, color: col, width: 44, textAlign: "right" }}>{s.bias > 0 ? "+" : ""}{s.bias}</span>
      </div>
      {s.why && <div style={{ fontSize: 10.5, color: T.ink3, marginTop: 2, marginLeft: 110 }}>{s.why}</div>}
    </div>
  );
}
  ...
  return (
    <Card pad={14}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
        <div>
          <div style={{ fontSize: 13.5, fontWeight: 700, fontFamily: T.mono }}>{f.t}</div>
          <div style={{ fontSize: 10.5, color: T.ink3 }}>{f.n}</div>
        </div>
        <div style={{ textAlign: "right" }}>
          <div style={{ fontFamily: T.mono, fontSize: 16, fontWeight: 700 }}>{f.px.toLocaleString()}</div>
          <span style={{ fontSize: 10.5, fontWeight: 700, color: biasColor(f.bias) }}>{biasLabel(f.bias)} bias</span>
        </div>
      </div>
      <div style={{ position: "relative", height: 100, marginLeft: 6 }}>
        ...
        {ticks.map((t) => (
          <div key={t.k} style={{ position: "absolute", left: 0, right: 0, top: `calc(${y(t.v)}% - 8px)`, display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontFamily: T.mono, fontSize: 10.5, color: T.ink2 }}>{t.v.toLocaleString()}</span>
          </div>
        ))}
  ...
  return (
    <div>
      ...
      <div className="om-stagger" style={{ padding: "12px 16px", display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <>
        {/* Bias meter + Lufi's read */}
        <Card pad={14} style={{ display: "grid", gridTemplateColumns: "280px 1fr", gap: 18, alignItems: "center" }}>
          <div>
            <div style={{ fontSize: 11.5, color: T.ink3, fontWeight: 600 }}>Today's composite bias</div>
            <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginTop: 2 }}>
              <span style={{ fontSize: 28, fontWeight: 800, letterSpacing: -1.2, color: toneCol }}>{toneWord}</span>
            </div>
            <div style={{ fontSize: 11.5, color: T.ink3, marginTop: 2, fontWeight: 600 }}>{toneDesc}</div>
            <div style={{ marginTop: 6 }}><BiasMeter score={overall} h={11} /></div>
            <div style={{ display: "flex", justifyContent: "space-between", fontFamily: T.mono, fontSize: 10, color: T.ink4, marginTop: 4 }}><span>BEARISH</span><span>{overall > 0 ? "+" : ""}{overall}</span><span>BULLISH</span></div>
            {vix && (
          ...
          <div style={{ borderLeft: `1px solid ${T.line2}`, paddingLeft: 18 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
              <LufiAvatar size={24} />
              <span style={{ fontSize: 12.5, fontWeight: 800, letterSpacing: -0.2 }}>Lufi's read</span>
            ...
            {read ? (
              <ul style={{ margin: 0, paddingLeft: 16, fontSize: 12.5, color: T.ink2, lineHeight: 1.5, listStyleType: "disc" }}>
                {read.split("\n").map((l) => l.replace(/^[•\-*]\s*/, "").trim()).filter(Boolean).map((l, i) => (
                  <li key={i} style={{ marginBottom: 3 }}>{l}</li>
                ))}
              </ul>
            ) : (
              <div style={{ fontSize: 12.5, color: T.ink2, lineHeight: 1.5 }}>
                Run analysis to fetch live sector ETF moves, compute pivot-based key levels for /ES, /NQ and the 10 majors, and have Qwen3 write today's directional read.
              </div>
            )}
          </div>
        </Card>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.3fr 1fr", gap: 10, alignItems: "start" }}>
          ...
          <Card pad={14}>
            ...
            {news.map((n, i) => (
              <div key={i} style={{ padding: "6px 0", borderBottom: i < news.length - 1 ? `1px solid ${T.line2}` : "none" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                  <span style={{ fontSize: 10, fontWeight: 700, color: T.ink2, fontFamily: T.mono }}>{n.src}</span>
                  <span style={{ fontSize: 9.5, color: T.ink4, fontFamily: T.mono }}>{n.min}</span>
                  <span style={{ marginLeft: "auto" }}>{sentChip(n.s)}</span>
                </div>
                <div style={{ fontSize: 12, color: T.ink, lineHeight: 1.4 }}>{n.t}</div>
              </div>
            ))}
          </Card>
        </div>
        ...
        <Card pad={14}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1.3fr repeat(4, 1fr) 90px", padding: "6px 6px", fontSize: 10, color: T.ink4, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.5, borderBottom: `1px solid ${T.line2}` }}>
          ...
          {levels.map((l) => (
            <div key={l.t} style={{ display: "grid", gridTemplateColumns: "1.3fr repeat(4, 1fr) 90px", padding: "7px 6px", alignItems: "center", borderBottom: `1px solid ${T.line2}`, fontFamily: T.mono, fontSize: 12 }}>
              ...
              <span style={{ textAlign: "right" }}><span style={{ fontSize: 10, fontFamily: T.sans, fontWeight: 700, color: biasColor(l.bias), background: biasColor(l.bias) + "18", padding: "2px 7px", borderRadius: 6 }}>{biasLabel(l.bias)}</span></span>
            </div>
          ))}
        </Card>
        </>
